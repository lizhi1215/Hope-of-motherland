...
def reduce_mean_layers(C2, C3, C4, C5):
...
def non_local_block(input_tensor, computation_compression):
	input_shape = input_tensor.get_shape().as_list()
	batchsize, height, width, channels = input_shape
	 # Embedded Gaussian instantiation
	input_branch_left = conv2d(input_tensor, channels, channels // 2, 1)
	input_branch_left = tf.reshape(input_branch_left, shape = [-1, height * width, channels // 2])

	input_branch_mid = conv2d(input_tensor, channels, channels // 2, 1)
	input_branch_mid = tf.reshape(input_branch_mid, shape = [-1, height * width, channels // 2])
	...
	transfer_left = tf.reshape(transfer_left, shape = [-1, height * width * input_branch_mid_shape[1]])
	transfer_left = tf.nn.softmax(transfer_left, axis = -1)
	transfer_left = tf.reshape(transfer_left, shape = [-1, height * width, input_branch_mid_shape[1]])

	input_branch_right = conv2d(input_tensor, channels, channels // 2, 1)
	input_branch_right = tf.reshape(input_branch_right, shape = [-1, height * width, channels // 2])
	if computation_compression > 1:
	...
	temp_out = tf.reshape(temp_out, shape = [-1, height, width, channels // 2])
